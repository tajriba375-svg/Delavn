...
function renderProfileForm() {
  ...
  form.innerHTML = `
    <section class="profile-v2-top">
      <div class="profile-v2-tools">⚙️</div>
      <div class="profile-v2-head-main">
        <div class="profile-v2-score">★ 5.0</div>
        <div class="profile-v2-user">
          <h3>${p.fullName || "عميل SwiftDrop"}</h3>
          <p>${p.phone || "+212 6XX-XXXXXX"}</p>
        </div>
        <div class="profile-v2-avatar">${firstLetter}</div>
      </div>
      <div class="profile-v2-pill">حسابي</div>
    </section>

    <section class="profile-v2-card profile-v2-mainlist">
      <button type="button" class="profile-v2-row" data-action="account">
        <span class="lead"><span class="ico">👤</span>المعلومات الشخصية</span>
        <span class="trail">‹</span>
      </button>
      <button type="button" class="profile-v2-row" data-action="payment">
        <span class="lead"><span class="ico">💳</span>طرق الدفع</span>
        <span class="trail">‹</span>
      </button>
      <button type="button" class="profile-v2-row" data-action="favorites">
        <span class="lead"><span class="ico">♡</span>المفضلة</span>
        <span class="trail">‹</span>
      </button>
      <button type="button" class="profile-v2-row" data-action="support">
        <span class="lead"><span class="ico">🎧</span>المساعدة والدعم</span>
        <span class="trail">‹</span>
      </button>
      <button type="button" class="profile-v2-row" data-action="business">
        <span class="lead"><span class="ico">💼</span>يسير للأعمال</span>
        <span class="trail">‹</span>
      </button>
      <button type="button" class="profile-v2-row" data-action="addresses">
        <span class="lead"><span class="ico">📍</span>العناوين</span>
        <span class="trail">‹</span>
      </button>
    </section>
    ...
    <section class="profile-v2-card profile-v2-form">
  ...
  form.querySelectorAll('[data-action]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'account') return toast('بيانات الحساب موجودة في النموذج أسفل هذه القائمة.');
      if (action === 'payment') return toast('الدفع المتاح حالياً: الدفع نقداً فقط.');
      if (action === 'favorites') return toast('المفضلة متاحة من الصفحة الرئيسية.');
      ...
      if (action === 'business') return toast('ميزة الأعمال ستتوفر قريباً.');
